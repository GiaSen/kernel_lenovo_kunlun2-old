...
enum Tfa98xx_Error show_current_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 2
	 ...
	 && tfa->verbose
		...
		pr_debug("%s  Current HW manager state: ", __func__);
...
enum Tfa98xx_Error tfaRunSpeakerBoost(struct tfa_device *tfa, int force, int profile)
{
	...
	pr_debug("%s  Startup of device [%s] is a %sstart\n", __func__, tfaContDeviceName(tfa->cnt, tfa->dev_idx), value ? "cold" : "warm");
	...
	case TFA_STATE_INIT_CF:      /* coolflux HW access possible (~initcf) */
		...
		do {
			err = (enum tfa_error)tfa98xx_dsp_system_stable(tfa,
						&ready);
			if (err != tfa_error_ok) {
				pr_err("run tfa98xx_dsp_system_stable falied %d\n", err);
				return err;
			}
			if (ready)
				break;
		} while (loop--);
		pr_debug("%s  after performed tfa98xx_dsp_system_stable()  err=%d ready=%d  loop=%d\n", __func__, err, ready, loop);
