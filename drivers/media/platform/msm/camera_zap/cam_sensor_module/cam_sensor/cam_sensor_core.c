...
//LENOVO_CUSTOM begin
#define DISABLE_CAM_CHIP_ID_DEBUG
//LENOVO_CUSTOM end
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	uint32_t chipid = 0;
//LENOVO_CUSTOM begin
#ifndef DISABLE_CAM_CHIP_ID_DEBUG
...
//LENOVO_CUSTOM begin
	CAM_INFO(CAM_SENSOR, "### reg=0x%x,read id: 0x%x expected id 0x%x:",
	...
	if (slave_info->sensor_id == 0x0576) {
		...
		CAM_INFO(CAM_SENSOR, "### <imx576> read chip revision: 0x%x ",
			chiprevison);
		...
		CAM_INFO(CAM_SENSOR, "### <ov8356> read chip revision: 0x%x ",
			 chiprevison);
		...
		CAM_INFO(CAM_SENSOR, "### <imx519> read chip revision: 0x%x ",
			 chiprevison);
	}
#endif
//LENOVO_CUSTOM end
