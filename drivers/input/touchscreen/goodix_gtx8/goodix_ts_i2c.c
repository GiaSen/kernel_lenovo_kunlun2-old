...
int goodix_avdd_en(struct goodix_ts_device *dev, int enable)
{
	if (enable) {
		ts_info("[gtp] avdd en\n");
		gpio_direction_output(dev->board_data->avdd_en_gpio, 1);
		udelay(15000);
	} else {
		ts_info("[gtp] avdd dis\n");
		gpio_direction_output(dev->board_data->avdd_en_gpio, 0);
		msleep(5);
	}
...
#define GTP_FP
#ifdef GTP_FP
#define GTP_FP_MATCH 10
#define GTP_FP_NOT_MATCH 2
static int goodix_fp_match = 0;
#endif
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	for (i = 0; i < touch_num - 1; i++) {
		...
		/* read bit5 to judge if big finger is needed to restrain */
		coords->slideRes = buffer[i * BYTES_PER_COORD + 2] & 0x20;
		if( coords->slideRes == 1) {
			/* handle the big finger proc */
		}
		...
		coords->p = coords->w;
		coords->length = buffer[i * BYTES_PER_COORD + 8];
		coords->width = buffer[i * BYTES_PER_COORD + 9];
#ifdef GTP_FP
		if (goodix_fp_match == 1) {
			coords->w = GTP_FP_MATCH;
		} else {
			coords->w = GTP_FP_NOT_MATCH;
		}
#endif
...
#ifdef GTP_FP
		if(unlikely((event_sta & 0x08) == 0x08)) {
			/* need to notify FP */
			ts_debug("GTP match\n");
			goodix_fp_match = 1;
		} else
