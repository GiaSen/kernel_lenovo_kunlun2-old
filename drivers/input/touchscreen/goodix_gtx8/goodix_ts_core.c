		...
		struct goodix_touch_data *touch_data)
{
	...
	if (touch_num >= 1 && touch_data->pen_down) {
		...
		if (touch_data->pen_coords[0].p == 0)
			input_report_abs(dev, ABS_MT_PRESSURE, 2);
		else
	...
	for (i = 0; i < ts_bdata->panel_max_id * 2; i++) {
		if (touch_num && i == id) { /* this is a valid touch down event */
			...
			/* ts_info("!!!!!!report finger  ,%d,%d,(%d,%d)",
				coords->x, coords->y, coords->w, coords->p); */
...
#if GOODIX_SECURE_TOUCH
    if( goodix_filter_interrupt() == IRQ_HANDLED ) {
        return IRQ_HANDLED;
    }
#endif
...
static void goodix_ts_esd_work(struct work_struct *work)
{
	...
	if (r < 0) {
		...
		hw_ops->avdd_en(core->ts_dev, 0);
		hw_ops->avdd_en(core->ts_dev, 1);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	/* TS reset pull to 0.6V if enable avdd
	 * Set reset to low first can aovid this scenario
	 */
	gpio_direction_output(ts_device->board_data->reset_gpio, 0);
	msleep(5);
	ts_device->hw_ops->avdd_en(ts_device, 1);
...
#if GOODIX_SECURE_TOUCH
    ts_info("goodix secure_touch init start");
    goodix_secure_data = kzalloc(sizeof(*goodix_secure_data), GFP_KERNEL);
    if (!goodix_secure_data) {
        ts_err("secure touch initalize failed: no memory!");
        goto out;
    }
    goodix_secure_data->core_data = core_data;
    goodix_secure_data->i2c_client = to_i2c_client(ts_device->dev);
    goodix_secure_touch_init();
    goodix_secure_init();
    goodix_secure_touch_stop(1);
    ts_info("goodix secure_touch init end");
#endif
