...
void gsx_gesture_en(int enable)
{
	int ret;
	if (enable == 1) {
	...
	struct goodix_ext_module *module)
{
	...
	if (QUERYBIT(gsx_gesture->gesture_type, temp_data[2])) {
		...
		input_report_key(core_data->input_dev, KEY_GESTURE_DC, 1);
		input_sync(core_data->input_dev);
		input_report_key(core_data->input_dev, KEY_GESTURE_DC, 0);
		input_sync(core_data->input_dev);
		goto gesture_ist_exit;
	} else if( temp_data[2] == 'F' )
	{
		/* detect touch down, need to notify FP */
	}else if( temp_data[2] == 'U')
	{
		/* detect touch up, need to notify FP */
	}else
