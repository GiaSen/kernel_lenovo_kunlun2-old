...
int read_mass_data(u8 addr, int byte_num, int *buf)
{
    ...
    FTS_TEST_INFO("packet num:%d, remainder:%d reg_addr = %d", packet_num, packet_remainder,addr);
...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    sprintf(filepath, "%s%s", FTS_ITO_DATA_FILE_PATH, file_name);
    if (NULL == pfile) {
...
int fts_test_entry(char *ini_file_name)
{
    ...
    printk("liufurong ito_TestResult_fts = %d\n",ito_TestResult_fts);
