...
static char version[40]="Focaltech-ft8719 05";
static char vendor_name[50]="Focaltech-ft8719";
...
static int fts_get_info(struct fts_ts_data *ts_data)
{
	int ret = 0;
	u8 fw_cmd[1] = { 0 };
	u8 vendor_id=0;
	...
	ret = fts_i2c_read_reg(client, FTS_REG_FW_VER, &fw_cmd[0]);
	if (ret < 0) {
		FTS_ERROR("read ft3518 fw ver from tp fail");
		return ret;
	}

	fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &vendor_id);
	if (ret < 0) {
		FTS_ERROR("read ft3518 vendor id from tp fail");
		return ret;
	}
    snprintf(version, sizeof(version),"VER:0x%02x ID:0x%02x",fw_cmd[0],vendor_id );
    snprintf(vendor_name, sizeof(vendor_name),"Focaltech-ft8719" );
...
/* Huaqin  add for tp hardware info query by limengxia at 2018/09/06 start */
#if FTS_TOUCH_EXT_PROC
#include <linux/proc_fs.h>
#define FTS_INFO_PROC_FILE "fts_info"
...
static ssize_t fts_proc_getinfo_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	snprintf(buf, 150, "IC=ft8719 module=TXD fw_ver= %d \n",fwver);
...
int32_t fts_extra_proc_init(void)
{
	...
	{
           printk( "[fts] %s() Couldn't create proc entry!",  __func__);
           return -ENOMEM;
	}
	else
	{
           printk( "[fts] %s() Create proc entry success!",  __func__);
	}
	return 0;
}
#endif
/* Huaqin  add for tp hardware info query by limengxia at 2018/09/06 end */
/* huaqin add for gesture by limengxia at 20180903 start */
...
#endif
/* huaqin add for gesture by limengxia at 20180903 end */
...
/* Huaqin add for ito test by liufurong at 20180830 start */
...
int fts_test_node_init(struct platform_device *tpinfo_device)
{
    ...
    if (0 != err)
    {
        printk( "[zhengwufocal] %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&tpinfo_device->dev.kobj, &zh_test_attribute_group);
        return -EIO;
    }
    else
    {
        printk("[zhengwufocal] %s() - sysfs_create_group() succeeded.", __func__);
    }
    return err;
}
...
/* Huaqin add for ito test by liufurong at 20180830 end */
/*****************************************************************************
*  Reprot related
*****************************************************************************/
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
			...
			/* huaqin del for TPlog by limengxia at 2018/10/11 start */
			...
			/* huaqin del for TPlog by limengxia at 2018/10/11 end */
        } else {
            uppoint++;
...
#ifdef CONFIG_PRODUCT_KUNLUN2
	//get TP and LCD info
	ret = fts_get_info(ts_data);
	if (ret) {
		FTS_ERROR("get TP FW version error");
	}
#endif
...
#endif
/* Huaqin add for ito test by liufurong at 20180830 start */
    fts_test_node_exit(&hwinfo_device);//zhengwu add
/* Huaqin add for ito test by liufurong at 20180830 end */
    kfree_safe(ts_data->point_buf);
    kfree_safe(ts_data->events);
...
//[ZUIP-3719][JD19][TOUCH] add by gongdb begin
    mdelay(100);
//[ZUIP-3719][JD19][TOUCH] add by gongdb end
...
/* huaqin add for JD2019-692 by limengxia at 20181105 start */
        fts_release_all_finger();
/* huaqin add for JD2019-692 by limengxia at 20181105 end */
        return 0;
    }
#endif
...
/* huaqin add for gesture by limengxia at 20180903 start */
		...
		FTS_ERROR("sleep suspend end  disable vsp/vsn\n");
#endif
/* huaqin add for gesture by limengxia at 20180903 end */
