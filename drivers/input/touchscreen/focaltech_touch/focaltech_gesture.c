...
*****************************************************************************/
#define KEY_GESTURE_U                           KEY_GESTURE_DC
...
/* huaqin modify for gesture by limengxia at 20180903 start */
    fts_gesture_data.mode = DISABLE;
/* huaqin modify for gesture by limengxia at 20180903 end */
    fts_gesture_data.active = DISABLE;
...
#endif
#if FTS_GESTURE_EN
void fts_gesture_en(int enable)
{
    if (enable) {
        FTS_INFO("[GESTURE]enable gesture");
        fts_gesture_data.mode = ENABLE;
...
void fts_tp_screen_switch(int enable)
{
    struct input_dev *input_dev = fts_data->input_dev;
	...
	if (enable)
		ret = fts_i2c_write_reg(client, 0x8c, 0x01);
	else
		ret = fts_i2c_write_reg(client, 0x8c, 0x00);

	if (ret < 0)
		FTS_ERROR("Could not write 0x8c");
	else
		FTS_INFO("Write 0x8c, %d bytes successful", enable);
