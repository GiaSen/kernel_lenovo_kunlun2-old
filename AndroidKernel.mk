...
$(KERNEL_CONFIG): $(KERNEL_OUT)
	...
	$(hide) if [ ! -z "$(KERNEL_CONFIG_EXTRA)" ]; then \
			echo "Overriding kernel config with '$(KERNEL_CONFIG_EXTRA)'"; \
			cat $(TARGET_KERNEL_SOURCE)/arch/arm64/configs/ext_config/$(KERNEL_CONFIG_EXTRA) >> $(KERNEL_OUT)/.config; \
			$(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(BUILD_ROOT_LOC)$(KERNEL_OUT) $(KERNEL_MAKE_ENV) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) oldconfig; fi
